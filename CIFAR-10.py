...
net = Net()
print(net)
lrrang = [0.0005,0.001,0.0015]
loss_list = [[] for i in range(3)]
j = 0
torch.save(net,PATH)
for i in lrrang:
    ...
    for epoch in range(epochnum):
        ...
        for i, data in enumerate(trainloader, 0):#enumerate将其组成一个索引序列，利用它可以同时获得索引和值,enumerate还可以接收第二个参数，用于指定索引起始值
            ...
            if i % 2000 ==1999:
                ...
                loss_list[j].append(running_loss/20)
                running_loss = 0.0
    ...
    j=j+1
#绘制图像
x = np.linspace(0, epochnum*6-1, epochnum*6)
plt.plot(x,loss_list[0],label='lr=0.0005');
plt.plot(x,loss_list[1],label='lr=0.001');
plt.plot(x,loss_list[2],label='lr=0.0015');
plt.legend();
plt.xlabel('batch')
plt.ylabel('loss', fontsize='large')
plt.title('不同学习率下的trainloss')
